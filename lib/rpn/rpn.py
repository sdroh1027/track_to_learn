...
def get_rpn_triple_batch_sidong(roidb, cfg):
    """
    prototype for rpn batch: data, im_info, gt_boxes
    :param roidb: ['image', 'flipped'] + ['gt_boxes', 'boxes', 'gt_classes']
    :return: data, label
    ...
    imgs, bef_imgs, aft_imgs, roidb = get_triple_image(roidb, cfg) # 'boxes', 'im_info' added in multiple roidbs

    im_array = np.concatenate(imgs,axis=0)
    bef_im_array = np.concatenate(bef_imgs,axis=0)
    aft_im_array = np.concatenate(aft_imgs,axis=0)
    ...
    data = {'data': im_array,
            'data_bef': bef_im_array,
            'data_aft': aft_im_array,
            'im_info': im_info_batch}
    label = {'gt_boxes': gt_boxes_batch}
