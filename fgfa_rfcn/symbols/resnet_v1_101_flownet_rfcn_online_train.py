...
class resnet_v1_101_flownet_rfcn_online_train(Symbol):
    def __init__(self):
        """
    ...
    def compute_weight(self, embed_flow, embed_conv_feat):
        ...
        weight = mx.symbol.sum(data=embed_flow_norm * embed_conv_norm, axis=1, keepdims=True , name='unnormalize_weight') #
    ...
    def get_aggregation_symbol(self, cfg):
        ...
        conv_feat = mx.sym.BilinearSampler(data=feat_cache, grid=flow_grid, name='warping_feat')  # warped results of all adjacent frames
        ...
        # compute weight
        cur_embed = mx.symbol.slice_axis(embed_output, axis=0, begin=cfg.TEST.KEY_FRAME_INTERVAL, end=cfg.TEST.KEY_FRAME_INTERVAL+1, name = 'cur_embed')
        cur_embed_copy = mx.sym.tile(cur_embed, reps=(data_range, 1, 1, 1))
        unnormalize_weight = self.compute_weight(embed_output, cur_embed_copy)
        ...
        # ROI Proposal
        rpn_cls_score_reshape = mx.sym.Reshape(
            data=rpn_cls_score, shape=(0, 2, -1, 0), name="rpn_cls_score_reshape") #'num_anchors' scores are concatenated to one channel with extra heights
        rpn_cls_prob = mx.sym.SoftmaxActivation(
            data=rpn_cls_score_reshape, mode="channel", name="rpn_cls_prob") # do softmax point by point (so, anchor by anchor), across channel(yes or no)
        rpn_cls_prob_reshape = mx.sym.Reshape(
            data=rpn_cls_prob, shape=(0, 2 * num_anchors, -1, 0), name='rpn_cls_prob_reshape') #return to original shape
        ...
        # bounding box regression
        bbox_pred_batch = mx.sym.Reshape(name='bbox_pred_reshape', data=bbox_pred, shape=(-1, 4 * num_reg_classes))

        # reshape output
        cls_prob = mx.sym.Reshape(data=cls_prob, shape=(cfg.TEST.BATCH_IMAGES, -1, num_classes),
                                  name='cls_prob_reshape')
        bbox_pred = mx.sym.Reshape(data=bbox_pred_batch, shape=(cfg.TEST.BATCH_IMAGES, -1, 4 * num_reg_classes),
                                   name='bbox_pred_reshape')

        # res5_sub_network
        num_instances = cfg.TEST.NUM_INSTANCES
        #rfcn_inst = mx.sym.Convolution(data=rfcn_feat, kernel=(1, 1), num_filter=7 * 7 * num_instances, name="rfcn_inst")
        rfcn_ibbox = mx.sym.Convolution(data=rfcn_feat, kernel=(1, 1), num_filter=7 * 7 * 4 * num_reg_classes,
                                        name="rfcn_ibbox")

        # dynamic symbol making for online instance learning
        rfcn_inst = []
        for i in range(num_instances):
            rfcn_inst_temp = mx.sym.Convolution(data=rfcn_feat, kernel=(1, 1), num_filter=7 * 7 * 1, name="rfcn_inst_%d" % i)
            rfcn_inst.append(rfcn_inst_temp)
        rfcn_inst = mx.symbol.Concat(*rfcn_inst, dim=1, name="rfcn_inst_concat")


        psroipooled_inst_rois = mx.contrib.sym.PSROIPooling(name='psroipooled_inst_rois', data=rfcn_inst, rois=rois,
                                                            group_size=7, pooled_size=7,
                                                            output_dim=num_instances, spatial_scale=0.0625)
        psroipooled_iloc_rois = mx.contrib.sym.PSROIPooling(name='psroipooled_iloc_rois', data=rfcn_ibbox, rois=rois,
                                                            group_size=7, pooled_size=7,
                                                            output_dim=8, spatial_scale=0.0625)
        inst_score = mx.sym.Pooling(name='ave_inst_scors_rois', data=psroipooled_inst_rois, pool_type='avg',
                                    global_pool=True,
                                    kernel=(7, 7))
        ibbox_pred = mx.sym.Pooling(name='ave_ibbox_pred_rois', data=psroipooled_iloc_rois, pool_type='avg',
                                    global_pool=True,
                                    kernel=(7, 7))

        # sub_classification
        inst_score = mx.sym.Reshape(name='inst_score_reshape', data=inst_score, shape=(-1, num_instances))
        inst_prob = mx.sym.SoftmaxActivation(name='inst_prob', data=inst_score)
        # sub_bounding box regression
        ibbox_pred = mx.sym.Reshape(name='ibbox_pred_reshape', data=ibbox_pred, shape=(-1, 4 * num_reg_classes))

        # sub_reshape output
        inst_prob = mx.sym.Reshape(data=inst_prob, shape=(cfg.TEST.BATCH_IMAGES, -1, num_instances),
                                  name='inst_prob_reshape')
        ibbox_pred = mx.sym.Reshape(data=ibbox_pred, shape=(cfg.TEST.BATCH_IMAGES, -1, 4 * num_reg_classes),
                                   name='ibbox_pred_reshape')

        #sliced_bbox = mx.symbol.slice_axis(name='sliced_bbox', data=bbox_pred_batch, axis=1, begin=3, end=8) # first value has no meaning in my code (originally it meant batch index)
        #new_embed = mx.sym.ROIPooling(data=cur_embed, rois=sliced_bbox, pooled_size=(cfg.TEST.EMBED_SIZE,cfg.TEST.EMBED_SIZE), spatial_scale=1.0) #
        #new_embed = mx.sym.Reshape(data=new_embed, shape=(cfg.TEST.BATCH_IMAGES, -1, 2048, cfg.TEST.EMBED_SIZE * cfg.TEST.EMBED_SIZE), name='new_embed')
        #new_embed = mx.sym.L2Normalization(data=new_embed, mode='channel') #channel option makes 2nd index (channel)
    ...
    def crop_embed(self, cfg):
        # using cur_embed & bbox_pred, make new instance wise embed layer
        # cur_embed is 4dim tensor
        # bbox[4:8] has (x1,y1) (x2,y2) coordinates. we have to slice it
        cur_embed = mx.sym.Variable('cur_embed')
        bbox = mx.sym.Variable('bbox_with_delta')

        sliced_bbox = mx.symbol.slice_axis(bbox, axis=1, begin=4, end=8)
        new_embed = mx.sym.ROIPooling(data=cur_embed, rois=sliced_bbox, pooled_size=(cfg.TEST.EMBED_SIZE,cfg.TEST.EMBED_SIZE), spatial_scale=1.0, name='new_embed')
        new_embed_normalized = mx.sym.L2Normalization(data= new_embed, mode='channel', name='new_embed_normalized')

        self.sym = new_embed_normalized
        return new_embed_normalized

    def crop_embed2(self, cfg):
        # using cur_embed & bbox_pred, make new instance wise embed layer
        # cur_embed is 4dim tensor
        # bbox[4:8] has (x1,y1) (x2,y2) coordinates. we have to slice it
        cur_embed = mx.sym.Variable('cur_embed')
